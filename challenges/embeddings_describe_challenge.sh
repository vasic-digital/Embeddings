#!/usr/bin/env bash
# embeddings_describe_challenge.sh
#
# Round-249 paired-mutation deep-doc challenge for Embeddings.
#
# Validates that:
#   1. The deep-doc ledger (docs/test-coverage.md) lists every exported
#      symbol from pkg/{provider,openai,cohere,voyage,jina,google,bedrock}.
...
#      against every provider Client via loopback httptest, byte-preserving
#      non-ASCII input through JSON marshal.
#   4. The README enumerates the seven packages and the round-249 anti-bluff
#      guarantees section.
#
...
if [ "${MUTATE}" -eq 1 ]; then
    ...
    # Plant a rename: EmbedBatch → EmbedBogus (real symbol must remain
    # absent from ledger to flag the drift).
    sed -i 's/EmbedBatch/EmbedBogus_MUTATED/g' "${TMP_LEDGER}"
    LEDGER_WORK="${TMP_LEDGER}"
    echo "=== Embeddings Describe Challenge (anti-bluff-mutate mode) ==="
else
    echo "=== Embeddings Describe Challenge (clean mode) ==="
fi
echo ""
...
if [ ! -f "${LEDGER_WORK}" ]; then
    ...
    pass "ledger present"
    if grep -q "round-249" "${LEDGER_WORK}"; then
        pass "ledger marked round-249"
    else
        fail "ledger missing round-249 marker"
    fi
...
CHECKED=0
MISSING=0
for pkg in provider openai cohere voyage jina google bedrock; do
...
# Section 4: runner builds + runs against every provider Client
echo ""
echo "Section 4: bilingual runner build + run (real loopback transport)"
if [ ! -f "${RUNNER}" ]; then
    fail "runner missing at ${RUNNER}"
else
    pass "runner source present"
    cd "${MODULE_DIR}"
    if go build -o /tmp/embeddings_round249_runner ./challenges/runner/ 2>/tmp/emb_build.log; then
        pass "runner builds"
        if /tmp/embeddings_round249_runner -fixtures "${FIXTURE}" > /tmp/emb_run.log 2>&1; then
            pass "runner exit 0 against every provider Client"
            if grep -q "PASS \[openai\]\[sr\]" /tmp/emb_run.log; then
                pass "openai Cyrillic (sr) round-trip"
            else
                fail "openai Cyrillic (sr) missing from runner output"
            fi
            if grep -q "PASS \[cohere\]\[ja\]" /tmp/emb_run.log; then
                pass "cohere Japanese (ja) round-trip"
            else
                fail "cohere Japanese (ja) missing from runner output"
            fi
            if grep -q "PASS \[bedrock-titan\]\[ar\]" /tmp/emb_run.log; then
                pass "bedrock-titan Arabic (ar) round-trip"
            else
                fail "bedrock-titan Arabic (ar) missing from runner output"
            fi
            if grep -q "PASS \[contract:" /tmp/emb_run.log; then
                pass "interface contract verified at runtime"
            else
                fail "interface contract check missing from runner output"
            fi
        else
            fail "runner exit non-zero — see /tmp/emb_run.log"
            sed -n '1,20p' /tmp/emb_run.log
        fi
    else
        fail "runner build failed — see /tmp/emb_build.log"
        sed -n '1,20p' /tmp/emb_build.log
    fi
    rm -f /tmp/embeddings_round249_runner
fi

# Section 5: README round-249 anti-bluff section
echo ""
echo "Section 5: README round-249 anti-bluff section"
...
if grep -q "round-249" "${README}"; then
    pass "README marked round-249"
else
    fail "README missing round-249 marker"
fi
